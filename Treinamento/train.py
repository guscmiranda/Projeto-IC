from SBCAS_2026.EnsembleFractal.model import criar_modelo
...
from SBCAS_2026.EnsembleFractal.dataset import load_data_from_folders, ImageDataset
...
def run_kfold(dataset_path, dataset_type, class_names, backbone="mobilenet", seed=SEED, t_f=None, t_name=""):
    ...
    for fold, (train_idx, val_idx) in enumerate(tqdm(skf.split(paths, labels), total=skf.get_n_splits(), desc="Folds")):
        ...
        train_dataset = ImageDataset(train_data, transform=t_f)
        val_dataset   = ImageDataset(val_data, transform=TRANSFORM_DICT["transform"])
...
def train_seeds(seeds, dataset_path, classes, transform, transform_name):
    ...
    for seed in tqdm(seeds, desc=f"Seeds ({transform_name})", leave=False):
        ...
        results[seed]['mobnet_orig'] = run_kfold(
            dataset_path, "originais", classes, backbone="mobilenet", seed=seed, t_f = transform, t_name=transform_name
        )
        torch.cuda.empty_cache()

        # EfficientNet-B0 - originais
        results[seed]['effnet_orig'] = run_kfold(
            dataset_path, "originais", classes, backbone="efficientnet_b0", seed=seed, t_f = transform, t_name=transform_name
        )
        torch.cuda.empty_cache()
